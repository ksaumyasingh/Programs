...
void wroung() //for generating formatted warning when input is wroung
{
	HANDLE h=GetStdHandle(STD_OUTPUT_HANDLE);
	system("CLS");
...
void copyFile()  //copying the midiator.txt file to hospital.txt file(now hospital.txt file is updated to its modified content)
{
    string row;
...
//derived class Login having base class User
class Login:public User
{
	protected:
		string phoneno,refid;
	public:
		void get_phoneno()//function for reading phone number and if input is wroung then again asking for reading phone number.
		{
			int f;
		...
		void display()//function for displaying login information if user already previously regestered with same number and if number is new then user can register.
		{
...
class Hospital //base class Hospital
{
	public:
		...
		void displayHospital(string pinDist)//function for displaying hospitalinformation through pincode or district name form Hospital file.
		{
			cout.setf(ios::left);
		...
		void updateHospitalEntries(string refno,int day)//for updating hospital entries by copying to another mediator file and recopying modified file to base Hospital file(hence Hospital file get updated).
		{
			string row;
...
//derived Appointment class having base class User
class Appointment: public Login
{
	public:

		int isAppointmentPresent(string adharno)//fuction for searching through aadhaarno if appointment is already scheduled or not by checking Appointment file.
		{
			string row;
		...
		void selectSlot(int slot=1)//fuction for storing time slot selected by user in Appointment file.
		{
			ofstream Appointment("appointment.txt",ios::app);
			if(slot==1)
			{
...
class BookYourAppointment:public Hospital,public Appointment
{
	...
	public:
		...
		void get_pincode()//function for reading pincode
		{
			system("CLS");
...
//CLASS certificate is to show the certificate of a vaccinated person
